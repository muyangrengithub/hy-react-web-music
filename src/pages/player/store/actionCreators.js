...
import { getSongDetail, getLyric, getSimiPlaylist, getSimiSong,getHotComment,getSongComment } from '@/services/player';
...
const changeSongCommentAction = (res) => ({
...
export const changePlaySongAction = (tag) => {
  return (dispatch, getState) => {
    ...
    getLyric(currentSong.id).then(res => {
      const lrcString = res.lrc && res.lrc.lyric
...
export const getSongCommentAction = (limit,offset) => {
  return (dispatch,getState) => {
    const id = getState().getIn(["player", "currentSong"]).id;
    if (!id) return;
